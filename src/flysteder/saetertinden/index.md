---
name: Sætertinden
id: saetertinden
status: utkast
region: Harstad og Kvæfjord
external:
  flightlog_id: 1599
  pgearth_id: null
  yr_id: null
level: null
training_site: false
categories:
- PG
season: Vår og sommer
wind_directions:
  primary:
  - S
  - SW
  - SE
  - E
  possible: []
  source: flightlog/lokalkunnskap
launch:
  lat: 68.58694
  lon: 16.36222
  source: flightlog
landings:
- name: Landing Hårvik
  lat: 68.56611
  lon: 16.41944
  primary: true
  source: manuell
parking:
- name: P hovedruta
  lat: 68.565455
  lon: 16.428403
  source: gpx
- name: P sørsiden
  lat: 68.569078
  lon: 16.397504
  source: gpx
access:
  parking_text: Ved idrettsplassen i Årbogen eller grustaket i Hårvika.
  route_text: Følg ryggen oppover mot toppen. Bratt de siste 200 høydemeterne.
  routes:
  - file: saetertinden-route-1.gpx
    name: hovedruta
    km: 5.9
    elevation_gain_m: 1080
    walk_time_min: 115
  - file: saetertinden-route-2.gpx
    name: via sørsiden
    km: 5.7
    elevation_gain_m: 1100
    walk_time_min: 110
elevation:
  launch_masl: 1095
  landing_masl: null
  difference_m: null
  source: flightlog
airspace:
- name: Evenes CTR
  nearby: true
  note: Meld fra til tårnet på Evenes før start, tlf. 67 03 41 23.
  source: flightlog
hazards:
- title: Evenes CTR
  text: Flyging fra toppen ned til Hårvika er tett inntil Evenes CTR. Meld fra til tårnet på Evenes før start, tlf. 67 03 41 23.
- title: Stupstart
  text: Noen retninger er stupstart. Mot sør er det loddrett stup etter en kort startstrekning, så ikke nøl når du først har begynt å løpe.
launches:
- directions:
  - SW
  categories: null
  text: Fin startplass med jevnt underlag og helning.
- directions:
  - SE
  - E
  categories: null
  text: Krever vind for baklengsstart, eller snø tidlig i sesongen. Alternativ start på ca. 850 moh.
images:
  overview: null
  launch: null
  landing: null
  air: null
  overview_credit: null
reviewed:
  by: null
  date: null
---

Høyt og flott, nesten 1100 moh, med mange fine landinger i Hårvika.

## Start

Om vinteren er det fint å starte flere steder. Om sommeren er store deler av toppen dekket av stein, så studer kart og meteogram før du går opp.

## Landing

Mange fine landinger i Hårvika.
