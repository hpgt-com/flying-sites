---
name: Elgen
id: elgen
status: utkast
region: Harstad og Kvæfjord
external:
  flightlog_id: 356
  pgearth_id: null
  yr_id: 1-293790
level: PP3
training_site: false
categories:
- PG
- SPG
season: Vår til høst
wind_directions:
  primary:
  - N
  - NE
  - E
  - SE
  - S
  - SW
  - W
  - NW
  possible: []
  source: flightlog/lokalkunnskap
launch:
  lat: 68.89083
  lon: 16.28194
  source: flightlog
landings:
- name: SPG-landing (ca.)
  lat: 68.89083
  lon: 16.3031
  primary: true
  source: manuell
parking:
- name: P kapellet
  lat: 68.91139
  lon: 16.269626
  source: gpx
- name: P Aun
  lat: 68.896727
  lon: 16.292437
  source: gpx
access:
  parking_text: Ved kapellet på Elgsnes. Stien opp starter her.
  route_text: Stien fra kapellet opp ryggen. De siste 100 høydemeterne er luftige. Gangbart fra midten av mai.
  routes:
  - file: elgen-route-1.gpx
    name: fra kapellet
    km: 3.5
    elevation_gain_m: 490
    walk_time_min: 50
  - file: elgen-route-2.gpx
    name: fra Aun (SPG)
    km: 4.1
    elevation_gain_m: 510
    walk_time_min: 60
elevation:
  launch_masl: 506
  landing_masl: 4
  difference_m: 502
  source: flightlog
airspace: []
hazards:
- title: Solgangsbris fra nord
  text: Kan gi kraftig turbulens og vindskjær, gjerne sammen med lave tåkeskyer. Lite vind på fjellet kan bety mye vind på landing.
- title: Mye vind og fallvind
  text: Pass på for mye vind. Det kan komme fallvind over Aunskardet ned mot landingen på sørsiden.
launches:
- directions:
  - N
  - NE
  - E
  - SE
  - S
  - SW
  - W
  - NW
  categories:
  - PG
  text: PG dekker alle retninger og kan starte over hele fjellet. Startplassene er tydelige når du kommer opp på toppen. V og NV gir stødig hang, godt for soaring.
- directions:
  - NE
  - E
  - SE
  - S
  - SW
  categories:
  - SPG
  text: SPG tar primært av på sørsiden av Elgen. Mot Ø og SØ er det stupstart øst for østtoppen.
images:
  overview: elgen-overview.jpg
  launch: null
  landing: null
  air: null
  overview_credit: Gammel flystedsoversikt
reviewed:
  by: null
  date: null
---

Ytterst i havgapet, med utsikt mot Grytøya og midnattsola. Fjellet dekker de fleste vindretninger, med egne starter for PG og SPG. Flott for soaring og kveldsturer.

## Start

Startene ligger rundt østtoppen (506 moh). Østsiden er ofte termisk morgen og formiddag i sol.

## Landing

PG lander nede ved kapellet på Elgsnes, der bilen står, eller på stranda ytterst ved Elgsnes.

SPG lander på jordet under SØ-starten, på sjøsiden av veien vest for det vestligste huset i Aun. Avklar bruk med naboen, og før onna: land i ytterkant. Flaggstanga med vimpel på gården viser vinden, og naboen liker å se folk fly.
