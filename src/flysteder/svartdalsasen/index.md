---
name: Svartdalsåsen
id: svartdalsasen
status: utkast
region: Harstad og Kvæfjord
external:
  flightlog_id: 1953
  pgearth_id: null
  yr_id: 10-1093809
level: null
training_site: false
categories:
- PG
season: Vår til høst
wind_directions:
  primary:
  - W
  - NW
  possible: []
  source: flightlog/lokalkunnskap
launch:
  lat: 68.75556
  lon: 16.42361
  source: flightlog
landings:
- name: Landing
  lat: 68.78833
  lon: 16.47528
  primary: true
  source: manuell
parking: []
access:
  parking_text: Ved skytebanen på Heggenkollen.
  route_text: Følg stien opp til Appelsinhaugen, og videre langs Svartdalsåsen til starten helt på sørsiden. Alternativt fjellveien fra Folkeparken forbi Musvannet.
  routes: []
  walk_time_min: 30
elevation:
  launch_masl: 360
  landing_masl: null
  difference_m: null
  source: flightlog
airspace: []
hazards: []
launches:
- directions:
  - W
  - NW
  categories: null
  text: Hang og termisk hang.
images:
  overview: svartdalsasen-overview.jpg
  launch: null
  landing: null
  air: null
  overview_credit: Gammel flystedsoversikt
reviewed:
  by: null
  date: null
---

Blant de beste termikkgeneratorene i området, ofte med hang samtidig. Samme retning som Kilhusåsen, men lengre å gå.

## Start

Starten er ganske bratt, men ryddet for busker og kratt. Mange flyr hit fra Kilhusåsen.

## Landing

Landingsbeskrivelse mangler.
