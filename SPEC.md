...
- Kart med alle starter som små vindroser. Luftromslag som i IPPC kan slås av og på i kartets lagvelger: TMA, CTR, militære områder, fare og restriksjon. Av som standard, lastes først når de slås på, klikk viser navn, klasse og grenser. Kreditering «Luftrom: openAIP … Ikke for navigasjon, sjekk IPPC». Termikklag (også på stedssidene): «Termikk: skyways» og «Termikk: hotspots» fra thermal.kk7.ch (CC BY-NC-SA 4.0, statistikk fra loggede flyturer, ikke varsel), av som standard. Ingen værradar, den sjekkes i Yr/Windy. Lag «Værstasjoner» (på som standard) fra `src/_data/stations.json` (navn, kilde, url, lat, lon, moh): symbol med popup og lenke «Se vinden nå» til stasjonens side. Viser ikke målinger (Holfuy-API krever nøkkel). Steder som ikke passer filteret tones ned. Klikk på en start viser et kort med rose, tagger, kort tekst, «Se hele stedet» og «Se på Flightlog».
